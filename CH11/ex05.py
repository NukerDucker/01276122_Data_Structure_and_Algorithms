...
def route(src, dest, map):
    ...
    while queue:
        ...
        for neighbor in map.get(current_station, []):
...
def main():
    ...
    input_str = input('Enter : ').split(',')
    src = input_str[0].strip()
    dest = input_str[1].strip()
    print(f'From: {src}')
    print(f'To: {dest}')
    print('Searching shortest route... This may take a while...')
    result = route(src, dest, tubemap)
